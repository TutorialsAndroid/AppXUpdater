...
internal object UtilsDisplay {
  ...
  fun showUpdateNotAvailableDialog(context: Context, title: String?, content: String?): AlertDialog {
    return AlertDialog.Builder(context)
      .setTitle(title)
      .setMessage(content)
      .setPositiveButton(context.resources.getString(android.R.string.ok)) { dialogInterface, i -> }
      .create()
  }
  ...
  fun showUpdateAvailableSnackbar(context: Context, content: String?, indefinite: Boolean?, updateFrom: UpdateFrom, apk: URL?): Snackbar {
    ...
    snackbar.setAction(context.getResources().getString(R.string.appupdater_btn_update)) { UtilsLibrary.goToUpdate(context, updateFrom, apk) }
    return snackbar
  }
  ...
  fun showUpdateAvailableNotification(context: Context, title: String, content: String?, updateFrom: UpdateFrom, apk: URL?, smallIconResourceId: Int) {
    ...
    val contentIntent = PendingIntent.getActivity(context, 0, context.packageManager.getLaunchIntentForPackage(UtilsLibrary.getAppPackageName(context)), PendingIntent.FLAG_CANCEL_CURRENT)
    val pendingIntentUpdate = PendingIntent.getActivity(context, 0, UtilsLibrary.intentToUpdate(context, updateFrom, apk), PendingIntent.FLAG_CANCEL_CURRENT)
